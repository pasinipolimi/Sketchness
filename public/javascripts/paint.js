...
function( Class,   Chat,   StateMachine,   Communicator,   Time,   Writer,   Painter,          $) {
	...
	$(function() {
		...
		var possibleGuesses = {
				...
				shirt: ["coat","cardigan","cape","hoodie","jacket","jumper","blazer","blouse","suit", "sweater", "sweatshirt", "t-shirt", "top", "vest"],
		...
		var GameFSM = new Class({
			...
			_proto: {
				...
				beginRound: function(sketcher) {
					...
					//randomly choose between bot (true) or human (false) player
					var botGame = userTest.sequence[userTest.count];
					userTest.count = userTest.count + 1;
				...
				timeUp: function() {
					...
					//if(Object.size(this.sketchness.players)==1) {
						...
						if(userTest.sequence[userTest.count-1]){
				...
				onentertaskDrawing: function() {
					...
					this.communicator.on({
						...
						timer: function(e, content) {
						    ...
						    that.clock.changeCountdown("task", 0);
						},
				...
				onentertaskDrawingBot: function() {
					...
					areaFunction = setInterval(function(){checkArea()}, 15000);
				...
				onenterimageViewing: function() {
					if(this.sketchness.guessed){
						this.write.top($.i18n.prop('guessed'), this.sketchness.word);
						this.sketchness.guessed = false;
					}
					else{
					...
					this.clock.setCountdown("solution", this.constants.solutionTime * Time.second, Time.second, this.write.time.bind(this.write), this.compileTest.bind(this));
				...
				/**
				 * Transition method for bot testing, time to compile
				 */
				compileTest: function() {
					this.write.top($.i18n.prop("botTest"));
					this.clock.setCountdown("solution", 5 * Time.second, Time.second, this.write.time.bind(this.write), this.nextRoundCall.bind(this));
				},
        ...
        onComplete: function(xhr,status){
            if(xhr.readyState === 4){
                if(xhr.status >= 200 && xhr.status < 300 || xhr.status === 304){
                    ...
                    var listPoints = result[0].history;
                	...
                	var j = 0;
                	for (var key in obj) {
                		timestamps[j] = parseInt(obj[key].time);
                		j++;
                	}
	...
	*/
	...
	function guessCloth(sk, possibleGuesses, name){
		...
		var nextGuess = function(){
			...
			//recursive call passing random time delay (between "minimum" and "maximum" seconds) for the next guess
			var maximum = 10;
			var minimum = 5;
