...
public class CMSOld {
	...
	private static List<ObjectNode> totQueueImages = Collections
			.synchronizedList(new LinkedList<ObjectNode>());
	private final static Integer queueSize = Play.application().configuration()
			.getInt("queueSize");
	private final static Integer queueThreshold = Play.application()
			.configuration().getInt("queueThreshold");
	...
	public static void taskSetInitialization(
		...
		if (tasksToAdd > 0) {
			retrieveImagesFromQueue(tasksToAdd, queueImages, roomChannel,
					uploadedTasks > 0);
	...
	private static void retrieveImagesFromQueue(final Integer tasksToAdd,
			final List<ObjectNode> queueImages, final Room roomChannel,
			boolean taskSent) {

		if (totQueueImages.size() < queueThreshold + tasksToAdd) {
			loadQueue();
		}

		for (int i = 0; i < tasksToAdd; i++) {
			ObjectNode guessWord = null;
			if (totQueueImages.size() > 0) {
				guessWord = totQueueImages.remove(0);
			}
			if (guessWord != null) {
				...
				if (!taskSent) {
					taskSent = true;
					LoggerUtils.debug("CMS", "Send task aquired for  rooomChanel: "
							+ roomChannel);
					sendTaskAcquired(roomChannel);
				}
			}
	...
	private static void loadQueue() {
		...
		final List<ObjectNode> provQueue = new ArrayList<>();
		...
		try {
			...
			params.put("limit", queueSize.toString());
		...
		for (final JsonNode item : retrievedImagesOrdered) {
			if (item.elements().hasNext()) {
				...
				provQueue.add(guessWord);

			}
		}
		totQueueImages = provQueue;
